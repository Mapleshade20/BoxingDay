...
Instruction *scanins()
{
  ...
  string valid_name[8] = {"inbox", "outbox", "add", "sup", "copyto", "copyfrom", "jump", "jumpifzero"};
  for (int j = 0; j < i; j++)
  {
    int k = 0;
    while (k < 8)
    ...
    if (k == 8 || (k <= 1 && program[j].is_box == false) || (k > 1 && program[j].is_box == true)) // 指令名称错误或box有数字或非box无数字
